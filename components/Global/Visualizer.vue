<template>
    ...
    >
        <div
            class="visualizer"
            ref="visualizer"
            @click="playOrStopMusique"
        >
            <audio
                class="visualizer_audio"
                controls
                :src="audio.src"
                crossorigin="anonymous"
                ref="visualizer_audio"
                :autoplay="sound"
            />
            <canvas
                class="visualizer_canvas"
                ref="visualizer_canvas"
            />
        </div>
    </transition>
</template>
...
export default {
    ...
    methods: {
        ...
        onEnter(el, done) {
            const { visualizer_audio } = this.$refs

            this.$TweenMax.to(visualizer_audio, {
                duration: 1.5,
